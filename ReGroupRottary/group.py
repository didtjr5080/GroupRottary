...
# print(all_people)
# print(all_people)
for line in execpt_list:
    all_people.remove(line)
...
# 한 리스트에 몇개씩 담을지 결정
n = int(max_count)
result = list(divide_list(all_people, n))
# print(result)
group_text_list = []
...
for i in range(0,len(result)):
    grop_text = str((i + 1)) + " 조: "
    for g in range(0,len(result[i])):
        grop_text = grop_text + " " + result[i][g]
    group_text_list.append(grop_text)
